import 'package:bootcamp_f32/features/app/listen_match_games/cleaning_listen_match_game/cleaning_listen_match_game.dart';
...
class CleaningListenMatchGameLevelList extends ConsumerStatefulWidget {
  const CleaningListenMatchGameLevelList({Key? key}) : super(key: key);

  @override
  ConsumerState<CleaningListenMatchGameLevelList> createState() => _CleaningListenMatchGameLevelListState();
}

class _CleaningListenMatchGameLevelListState extends ConsumerState<CleaningListenMatchGameLevelList> {
  List<String> levels =
  [
  ...
  Widget build(BuildContext context) {
    final data = ref.watch(cleaningListenMatchGameServiceProvider);
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
                  ...
                  return InkWell(
                    onTap: () {
                      if(data.lock[index] == 'assets/images/level_list/open_lock.png'){
                        ...
                        Navigator.of(context).push(MaterialPageRoute(builder: (context) => const CleaningListenMatchGame(),));
                      }else{
