import 'package:bootcamp_f32/features/app/make_logic_game/emotions_make_logic_game/emotions_make_logic_game.dart';
...
class EmotionMakeLogicGameLevelList extends ConsumerStatefulWidget {
  const EmotionMakeLogicGameLevelList({Key? key}) : super(key: key);

  @override
  ConsumerState<EmotionMakeLogicGameLevelList> createState() => _EmotionMakeLogicGameLevelListState();
}

class _EmotionMakeLogicGameLevelListState extends ConsumerState<EmotionMakeLogicGameLevelList> {
  List<String> levels =
  [
  ...
  Widget build(BuildContext context) {
    final data = ref.watch(emotionsMakeLogicGameServiceProvider);
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
                  ...
                  return InkWell(
                    onTap: () {
                      if(data.lock[index] == 'assets/images/level_list/open_lock.png'){
                        ...
                        Navigator.of(context).push(MaterialPageRoute(builder: (context) => const EmotionMakeLogicGame(),));
                      }else{
