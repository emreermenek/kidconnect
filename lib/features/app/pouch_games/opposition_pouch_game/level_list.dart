...
import 'package:bootcamp_f32/features/app/pouch_games/opposition_pouch_game/opposition_pouch_game.dart';
...
class OppositionPouchGameLevelList extends ConsumerStatefulWidget {
  const OppositionPouchGameLevelList({Key? key}) : super(key: key);

  @override
  ConsumerState<OppositionPouchGameLevelList> createState() => _OppositionPouchGameLevelListState();
}

class _OppositionPouchGameLevelListState extends ConsumerState<OppositionPouchGameLevelList> {
  List<String> levels =
  [
                  ...
                  return InkWell(
                    onTap: () {
                      if(data.lock[index] == 'assets/images/level_list/open_lock.png'){
                        for(int i = 0; i < 12; i++){
                          if(i == index){
                            data.imageIndexList = List.generate(36 - i*3, (index) => index);
                          }
                        }
                        data.currentLevel = index;
                        Navigator.of(context).push(MaterialPageRoute(builder: (context) => const OppositionPouchGame(),));
                      }else{
